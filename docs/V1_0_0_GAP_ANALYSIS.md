...
**Explicitly out of 1.0.0 product leaf** (track as **1.1 / 2.0** for cabal-only):
...
**In-scope memory stack (parallel track, not “later maybe”):**

| Layer | Repo | Role for cabal 1.0 |
|-------|------|--------------------|
| L1 cites | **tero-mcp** ← **tero-rs** | Cited corpus / index query (never silent) |
| Embedded / domain memory | **memory-gate-rs** → tero-rs memory tools → tero-mcp | Dense learn/retrieve when enabled |
| Session + **legitimate RAG** | **context-mcp** | First-class early target: real embeddings + retrieve (no pseudo-RAG claims) |

Rationale: 1.0 ships a **trustworthy leaf** that can **act on repos** and **pull memory** from the compose stack. Swarm orchestration stays post-1.0; **memory plumbing is not deferred**.

### Execution order (maintainer direction 2026-07-21)

1. **Tools usable** — write/apply_patch + multi-step loop + verify (E1 → E2)  
2. **Memory stack usable** — context-mcp real embedder path; tero-rs + memory-gate-rs feeding tero-mcp; cabal facade consumes all three honestly  
3. **Surfaces / ship** — stream, TUI, notify, packaging (E4–E8)
---
...
flowchart TB
  E0["E0 foundation"] --> E8
  E1["E1 tools write"] --> E2["E2 verify"]
  E2 --> E3["E3 budgets / package"]
  E3 --> E8["E8 ship"]
  E1 --> E6["E6 TUI"]
  E2 --> E6
  E4["E4 stream"] --> E6
  E5["E5 tero / session"] --> E6
  E5 --> E7["E7 notify / HITL"]
  E7 --> E8
```
